---
name: pet-ad-skill
description: 面向欧美社媒的宠物短剧、剧情带货、UGC开箱和身份翻盘短片。总控加载全局核心规则、项目事实及单一主风格；逐镜继承空间完成态、预演物理接触、设计摄影构图和声音，并在最终提示词中落实反应镜头、后果与完整结尾。
---

# 宠物广告skill｜总控路由

**本文件只管调用顺序、规则优先级与交付闸门。** 规则母版在 `references/CORE_RULES.md`；各风格只写创意增量，不重写全局规则。

## 1｜加载顺序与优先级

每次调用先读 `references/CORE_RULES.md`，再根据请求载入：

1. **全局母版**：`references/CORE_RULES.md` 明确 G-01～G-11 的归属文件与强制适用阶段。执行分镜／Prompt任务时按母版加载 00～10 公共模块；成片返修还须加载回归案例。
2. **本次项目事实**：当前上传的参考图／视频、已确认的角色与场景，按 `references/PROJECT_CONTRACT.md` 建立项目状态；没有的参考不能虚构。
3. **一个主风格**：从 `references/styles/` 选取。混合时只允许副风格补局部机制；不能用风格规则覆盖全局核心或项目事实。
4. **交付验收**：`evaluation/REGRESSION_CASES.md` 与 `evaluation/CROSS_STYLE_REGRESSION.md`；发现失败回到最早失效镜头，后续逐镜重新继承。

优先处理当前用户明确给出的事实、规格与创意例外；创意例外必须明确描述视觉变化过程，不得以“风格化”为借口省掉空间位移、道具交接或镜头接续。全局核心对所有风格生效，风格文件仅补充独有的故事节拍与表现方式。

## 2｜选择主风格

| 用户任务 | 加载文件 |
|---|---|
| 宠物IP、小事件大叙事、荒诞英雄 | `references/styles/ip-story-superhero-absurd.md` |
| 宠物剧情带货、产品体验 | `references/styles/commerce-story-product.md` |
| 宠物UGC开箱、产品发现 | `references/styles/ugc-unboxing.md` |
| 欧美竖屏豪门、复仇、身份翻盘 | `references/styles/us-vertical-revenge-identity-flip.md` |

没有创意：基于账号与目标构想一版大纲；已有故事：保留核心事件，只修因果与生成风险。使用“之前的金毛”时从 `references/00-project-defaults.md` 读取默认IP；用户新参考明确取代旧身份时以当前参考为准。

## 3｜生产流程：边写边预演，不再事后补锁

`选风格与项目事实 → 大纲／触发—反应规划 → 逐镜循环〔冻结全员末态＋当前运动者／原地者 → 同步预演画面／位置／构图／光影 → 验证刺激与动作可达 → 确定机位／景别／运镜／【构图】 → 冻结本镜全员末态〕 → 最短Prompt编译 → 声音／结尾核算 → 验收 → 成片局部返修`

**逐镜循环不可拆成“先写完所有漂亮分镜，最后再检查连续性”。** 下一镜首态必须等于上一镜末态；角色暂时出画仍保留位置与运动。摄影机换位不能替角色完成位移；构图不能重摆演员。具体执行见 `references/01-spatial-state-chain.md`、`references/02-physical-previsualization.md`、`references/08-perception-reaction-gate.md`、`references/09-four-layer-shot-gate.md`。后台对每镜联审画面、位置、构图、光影；只输出当前镜头必要的真实站位、关键地标和动作，不增加四层提示词附录。未被指定移动的配角即使出画仍保留原位。**空间锁只约束高风险位移段：位移完成并建立同地锚点后，恢复低机位、斜前反打、前景遮挡、景别跳变、短推拉等视觉变化与连续表演；不允许用人物换位制造变化。** 镜头中出现惊吓／认出／身份揭示等转折时，先锁定角色看向哪里、刺激从哪侧进入，再安排第一身体反射；不得为了拍正脸，让角色背对它正在受到惊吓的东西。

大纲先设计事件与人物关系，并按照公共 G-05 为默认30秒带对白短剧安排至少12个有效发声轮次；**大多数轮次必须是彼此承接的自然口语意群，不能用连续单词／两词碎片凑数量。** 用户明确无对白、纯静音或特别限制时按实际任务处理。创意允许虚实结合时，同步运行 G-11：优先寻找“真实环境保持正常＋一个不可能但连续可见的异常机制”，明确触发、传播、中间态、完成态和现实见证者。分镜／最终Prompt每镜均有 `时间＋景别＋观察机位／运镜＋【构图】＋当前位置／动作＋必要台词／声音`。每句分配说话者、剧情功能和时间窗；角色说话的口型、词数、换人、动作及尾部停留必须实际容得下。

## 4｜交付格式与最终闸门

- **只要大纲**：仅给故事结构；不提前堆摄影术语。
- **要导演分镜**：逐镜现状、可见位移、摄影角度／运镜、`【构图】` 和声音；对转折镜额外完成感知—反应账本，确认刺激来源／朝向／第一反射与识别有独立可读过程。
- **要视频Prompt**：规格 → 参考职责 → 必要角色／声线／持续环境 → 连续时间轴（每镜含【构图】）→ 一条持续BGM及事件音；按 `references/07-prompt-compiler.md` 去除抽象目标词。不在末尾追加“空间锁”代替逐镜动作。
- **已生成成片**：先核验实际文件，再对照分镜查穿帮；保留成功镜头，优先局部返修。

最终静默检查：`参考与身份 → G-05有效对白＋语义承接＋情绪递进 → 逐句容量／口型 → G-10画面＋全员位置＋构图＋光影四层接力 → 谁移动／谁原地 → 跨区域真实位移和高风险近景地标 → G-09感知／第一反射 → G-11现实底盘＋异常触发／传播／中间态＋镜头声画耦合（命中时） → 物理接触／机位 → BGM → Reaction → Consequence → Final State → 约1秒停稳`。对白不允许靠拆分一句、孤立关键词、拟声或在末尾补清单凑数量。

最后一句狠话／产品卖点／Reveal不是终点；结尾要看见反应、后果真的发生、角色关系完成最后一回合，再进入完成态。不能用“双方安静站好／突然和解”代替关系落地；风格可改变结尾呈现，不能删除结尾闸门。

## 5｜维护规则：改一处，所有风格继承

- 发现跨风格问题，先按 `references/CORE_RULES.md` 定位唯一规则拥有者，**只修改公共模块**；风格文件只记录该风格额外案例，不复制核心正文。
- 给修订规则分配／保留 G-ID，补充 `evaluation/REGRESSION_CASES.md` 真实失败案例，按 `evaluation/CROSS_STYLE_REGRESSION.md` 为每个现有风格检查适用结果。
- 新风格只新增 `references/styles/<style>.md` 并登记到本路由及跨风格矩阵，不复制 01～09 模块。
- 修改后运行 `evaluation/validate_structure.py`（静态结构检查），再做跨风格逐镜推演；静态通过不等于视频生成通过。
- GitHub `main` 的 `pet-short-drama-skill/` 为 SSOT，先同步母版，再更新其他平台副本；禁止以旧压缩包反向覆盖。
