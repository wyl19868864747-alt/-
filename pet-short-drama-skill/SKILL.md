...
**逐镜循环不可拆成“先写完所有漂亮分镜，最后再检查连续性”。** 下一镜首态必须等于上一镜末态；角色暂时出画仍保留位置与运动。摄影机换位不能替角色完成位移；构图不能重摆演员。具体执行见 `references/01-spatial-state-chain.md`、`references/02-physical-previsualization.md`、`references/08-perception-reaction-gate.md`、`references/09-four-layer-shot-gate.md`。后台对每镜联审画面、位置、构图、光影；只输出当前镜头必要的真实站位、关键地标和动作，不增加四层提示词附录。未被指定移动的配角即使出画仍保留原位。镜头中出现惊吓／认出／身份揭示等转折时，先锁定角色看向哪里、刺激从哪侧进入，再安排第一身体反射；不得为了拍正脸，让角色背对它正在受到惊吓的东西。
