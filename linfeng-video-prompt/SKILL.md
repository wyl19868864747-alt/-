...
把导演判断编译为**高信号、低污染、一读成像**的视频提示词。默认原则：**全局信息只说一次，分镜只写变化；详细写事件与动作，少写摄影说明书。**

## 固定链路

`Contract Lock → Route → Event Compile → Global Baseline → Minimal Prompt Compile → Independent Judge`
...
执行优先级：
`用户明确要求 > 已确认事实/参考资产 > 核心事件/证据 > 动作/情绪变化 > 空间连续 > 镜头响应 > 光影成像 > 技术术语`
...
| 新写、完整重写、光影/质感返修 | `references/camera-light-quality-baseline.md` |
...
视频的详细度优先给**时间、事件、动作和状态变化**，不是摄影术语。

关键Beat默认压成：
`起始状态 → 触发 → 可见动作/变化 → 结果 → 必要的镜头响应`
...
设备名必须对应可见结果；不把 IMAX、ARRI、UE5、Octane、VFX、8K、HDR 堆成“高级套餐”。通常一句媒介/质感 + 一句主光/色彩就够。

人物、产品、服装、空间、主光没有变化时，分镜不得重复介绍。

## 5. Prompt Compile｜Shot Specific
...
3. **【锁定】**：只写会漂移的身份/产品/空间/参考事实；没有就省略。
4. **【时间轴】**：按观看顺序写。每段优先 `事件 → 动作/变化 → 结果 → 必要镜头响应`。
5. **【声音/限制】**：只有需要时出现；只写准确对白、关键SFX或2–4个高风险限制。
...
交付前逐句检查：
1. 是否直接改变事件、动作、表演、空间、镜头、光色、材质、时间、文字或声音？
2. 是否已经在全局块说过？
3. 是否只是把一个有效概念解释得更长？
4. 是否为了显得专业加入设备/摄影术语？
...
Judge只检查失败点：事实、核心事件、可见状态变化、空间/接触/连续性、光影可读、镜头负载、Prompt冲突与冗余。FAIL时只修对应模块，不因局部问题整条推倒重来。
