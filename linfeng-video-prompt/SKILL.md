...
默认原则：**主体和事件先说清，动作与承接写够；每个明确分镜都用最短短语写一条运镜或摄影机状态，并融入动作句；光影、材质、景深、设备与物理真实性只在能直接改善当前镜头时加入。**
...
4. **【时间轴】**：按观看顺序写。每段优先 `事件 → 动作/变化 → 结果 → 简短运镜`。
...
- 每个明确分镜必须写**一条简短运镜或摄影机状态**：推近、后拉、跟随、固定机位、甩镜、环绕等；直接融入事件与动作句，不另起长段解释原因。
...
8. 是否每个明确分镜都有一条简短运镜，且与前后动作和落幅连贯？
