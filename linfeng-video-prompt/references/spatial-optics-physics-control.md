...
所有多镜任务逐镜执行 `prompt-compilation-and-consistency.md` 第4节，空间图复杂度门不能跳过状态交接；需要精确控制的单镜也后台建立 `First Frame State` 与 `End Frame State`。
...
多镜头时，下一镜默认从上一镜 `End Frame State` 合理延续，不重置动作、不瞬移、不无因改变距离、手别或物体状态。跨区域／往返／多人一动一静时，再执行 `spatial-handoff-lock.md` 的运动所有权、固定观察走廊与到达锚点；摄影机换角度不能替画外配角移动。
...
三层协议是后台规划结构，最终模型Prompt不输出独立空间位置锁。它只用于空间关系会持续变化或容易互相冲突的结构化任务，不用于典型4–10秒、单人、单场景、单动作链的简单片段。是否启用以控制难度为准，不以时长单独判断。
...
同一句只使用一个坐标系。房间/实体方位回答世界位置，画面左右回答当前构图，俯视图只用于后台检查；禁止把“房间东北角、俯视图左下、画面右侧、保持原位”混写为一条指令。发生跨区域移动、换位或新轴建立时，在动作发生的分镜中写出通道/转身/绕行和新观察关系，继承道具及角色状态；不得用另起空间段落替代移动过程。
...
后台确认沙发与壁炉相对、A/B身份和唯一药盒；相机始终在两人连线的壁炉侧。前台只写必要关系：

```text
0–2秒：壁炉侧斜前全景极慢前推，A坐在沙发近门一端，右手持药盒朝另一端的B伸出，B转头看盒子。
2–4秒：同侧正侧近景轻微横移，A右手仍托住两人之间的药盒，B左手接稳后A才松手。
4–6秒：同侧斜后全景极慢前推，B仍坐在沙发另一端，左手把接过的药盒收至膝上，抬眼回应已收回右手的A。
```

后台逐对核实同侧机位真实换位及角差；全局不复述站位，关键归属/接触在所属分镜内可读。
...
当 TS 涉及爆炸、冲击波、撞击、坍塌、能量释放、重物砸地、巨物落地或其他需要明显力量感的高动态事件时，自动启用本规则。禁止主要依赖“震撼、猛烈、史诗、炸裂、冲击感强”等抽象形容词表达效果，必须优先把冲击写成可见、可追踪的因果反馈链：

`事件节点 → 瞬时视觉反馈 → 力量传播 → 主体/环境受力 → 摄影机反馈 → 衰减恢复`

执行要求：
- **事件节点**：明确爆炸、撞击、落地或能量释放发生的瞬间，建立清楚时间锚点；
- **瞬时视觉反馈**：优先写第一帧可见变化，如短暂闪白、火光瞬间爆开、接触点炸尘、强光迅速达到峰值；
- **力量传播**：尽量交代起点、方向、形态和速度，如“冲击从中心高速向外扩散”“环形冲击波贴地扫开”；
- **环境受力**：不要直接说“力量很大”，而要通过尘土、碎石、金属残片、布料、头发、树叶、水面、车辆或建筑产生符合方向的受力反馈来证明力量；
- **主体受力**：若人物、车辆或关键道具处于作用范围内，必须让其姿态、重心、衣物、悬挂件或运动轨迹对冲击产生合理响应，不得完全无视冲击，也不得无因瞬移；
- **摄影机反馈**：需要震屏时写变化曲线而非静态标签。优先写“冲击瞬间第一下震幅最大 → 快速衰减 → 短暂余震 → 恢复稳定”，避免只写“镜头剧烈晃动”；
- **衰减恢复**：高能事件不能持续停在峰值，需根据画面写火焰膨胀减速、烟尘弥散、碎片落地、余波减弱、摄影机回稳等后续状态。

核心书写原则：**不要告诉模型“这个画面很有冲击力”，要告诉模型“冲击发生后，整个世界具体发生了什么”。** 动态效果优先描述过程曲线和物理结果，形容词只做辅助。

时间控制上，行为曲线优先于过度精确的小数秒。除非目标模型对局部时码控制可靠，否则优先写“短暂、第一下最强、随后快速衰减并回稳”；需要数字时可作为近似参考，不把 `0.3秒` 等数值当作绝对帧级保证。
...
- 是否写清力量传播方向或作用范围；
- 是否有符合受力方向的主体/环境反馈；
- 需要震屏时是否存在先强后弱的镜头反馈曲线；
- 是否写出峰值后的衰减或恢复；
- 是否出现穿模、瞬移、无因漂浮、受力方向矛盾或物体状态跳变。

若用户只写“来一个炸裂/震撼/有冲击力的效果”，Skill 应自动把抽象目标编译为上述因果链，不原样堆叠形容词。
...
连续性错误先回到相关分镜的继承状态、动作路径与机位重编；不在尾部补限制词。其他单镜光学/材质风险才按目标状态写一条局部修正。
...
`事实/必要拓扑与身份 → 上一镜完成态（首镜取实际首帧） → 当前空间关系与动作因果 → 可见观察位置 → 景别/机位/短运镜 → 必要光学/光线/声音 → 当前完成态 → 下一镜`

本链与编译器第4节共用同一状态，不另建第二套账本；复杂度只控制后台展开量。最终按时间轴逐镜写必要空间关系，不下发空间图、状态表或独立限制块。
