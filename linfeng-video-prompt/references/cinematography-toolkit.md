...
5. 后台完成全部技术选型；最终提示词第三部分只用一句 `【全片视觉与技术基线】` 转译最重要的可见结果。镜头段只写真实变化项，不重复全局设置。
6. 生成式视频模型不会真实调用这些设备或软件。专业名词必须继续转译为透视、景深、高光、肤色、反射、视差、运动模糊、接触阴影和颗粒等可见结果。

## 2. 后台完整技术方案与前台一句基线

从零生成或重新设计摄影时，在后台按任务填写以下字段。没有对应技术的字段写成可见的实拍状态，不虚构工具。这份完整方案用于判断，不直接整段复制到成品。

```text
成像路径：实景实拍 / CG 产品 / 虚拟场景 / 实拍与 VFX 混合。
摄影机：机型或成像观感、传感器格式、记录/色彩曲线。
镜头：镜头系列、主焦段、微距/变焦/变形宽银幕、滤镜。
曝光运动：帧率、快门角、T 值、EI/ISO、白平衡；高帧率段单列。
运动系统：三脚架、Dolly、Slider、Steadicam、TRINITY、Ronin、Technocrane、Bolt；路径、速度、缓入缓出。
焦点：起始焦点、拉焦对象、景深和对焦速度。
灯光：主光方位、高度、软硬、色温；反弹、补光、负补、轮廓、实用灯与背景层次。
曝光色彩：肤色/产品色、亮部滚降、黑位、对比、主色与强调色、Log/ACES/输出观感。
材质画质：皮肤、布料、玻璃、液体、金属、毛发、建筑；颗粒、噪点、锐化、运动中稳定性。
渲染与 VFX：主渲染器、物理模拟、跟踪/抠像/合成的唯一职责；纯实拍写实拍物理成像。
```
...
```text
【全片视觉与技术基线】
[成像与透视]；[主要运动质感]；[色彩语法及肤色/产品色保护]；[主光来源、世界方向、软硬与色温]；[关键材质及高光/暗部结果]；[必要渲染/合成职责]。
```

不列摄影机、镜头、灯具、运动系统和软件清单；只有某个专业技术直接改变可见结果且无法用更短语言替代时才保留一次。
