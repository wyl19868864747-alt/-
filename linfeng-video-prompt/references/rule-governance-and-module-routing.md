...
5. 结构检查跑 `scripts/audit_module_routing.py` 与 Skill Creator 验证；审计必须保证 `camera-light-quality-baseline.md` 位于写／重写 Prompt 的强制读取链，而不只是路由表里的可选项。行为用例覆盖主要触发、混合任务及不应误触发的简单场景，按改变范围选测。
